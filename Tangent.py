...
def Algorithm():
	...
	def Consume(x):
		...
		TTL = 0
		for i in range(0, len(Constant.DataArr)):
			y0 = x * (Constant.DataArr[i][0] - 160.97429999999994) + 53.915799999999976
			TTL += pow(y0 - Constant.DataArr[i][1], 2)
		return TTL / len(Constant.DataArr)
	...
	for kase in range(0, TTLkase):
		...
		#print(Jmed, Jmin, J[kase],alpha[kase+1], beta[kase+1], end = "\n")
